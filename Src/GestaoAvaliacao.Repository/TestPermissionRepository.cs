        ...
        public async Task<List<TestPermission>> GetPermissionByTest(long Test_Id, Guid? gru_id)
        {
            var sql = @"SELECT 
                        ...
                        FROM Synonym_Core_SYS_Grupo Gru WITH(NOLOCK)
	                        INNER JOIN TestPermission TP WITH(NOLOCK)
            ...
            {
                cn.Open();
                var result = await cn.QueryAsync<TestPermission>(sql.ToString(), new { test_id = Test_Id, sis_id = Util.Constants.IdSistema, gru_id = gru_id, vis_id = Util.EnumSYS_Visao.Administracao });
