...
*/
var edicoesComTurmasAmostrais = ["2017", "2018"];
var questionarios = [
    ...
    "12",/*Questionário Professor*/
    "13",/*Questionário Assistente de Diretoria*/
    ...
    "18",/*NOVO Questionário dos Alunos do 3º ano ID 18*/
    "19",/*NOVO Questionário dos Alunos do 4º ao 6º ano ID 19*/
    "20"/*NOVO Questionário dos Alunos do 7º ao 9º ano ID 20*/
];
/**
-----MSTECH-----
...
function onDeviceReady() {
    try {
        ...
        */
        $.ajax({
            ...
            success: function (data) {
                ...
                */
                //O usuário tem acesso apenas ao questionário 12 (Professor). Ele não deve acessar o relatório de acompanhamento.
                if (provaSP_configuracoes.configuracoes.RelatorioAcompanhamentoVisivel && (Usuario.AcessoNivelDRE ||
                    Usuario.AcessoNivelSME || Usuario.Diretor || Usuario.AssistenteDeDiretoria || Usuario.Supervisor)) {
        ...
        */
         ...
         *Adicionado novo questionário do Assistente de Diretoria - ID 13
         *Adicionado novo questionário do Auxiliar Técnico da Educação - ID 14
         *Adicionado novo questionário do Aluno - IDs 18 ao 20
        ...
        */
        ...
        if (Usuario.Aluno) {
            ...
            if (turma_ano == 3) { $("#divAbrirQuestionarioID_18").show(); }
            else if (turma_ano >= 4 && turma_ano <= 6) { $("#divAbrirQuestionarioID_19").show(); }
            else if (turma_ano >= 7 && turma_ano <= 9) { $("#divAbrirQuestionarioID_20").show(); }
        ...
        if (Usuario.AssistenteDeDiretoria) {
            $("#divAbrirQuestionarioID_13").show();
        }
...
function aplicarBIB() {
    try {
        ...
        */
        if (questionarioId_atual >= 18 && questionarioId_atual <= 20) {
            var turma_ano = parseInt(Usuario.Ano);
            if (Usuario.Ano == null) { turma_ano = 0 }

            //Aplica o BIB correto baseado no RF do usuário. Cada BIB corresponde a um caderno de respostas.
            if (turma_ano >= 3 && turma_ano <= 6) { qtdCadernos = 5; }
            else { qtdCadernos = 6; }
        }
        else if (questionarioId_atual == 12) { qtdCadernos = 5; }
        else if (questionarioId_atual == 3) { qtdCadernos = 7; }
        else if (questionarioId_atual == 13) { qtdCadernos = 4; }
        else { qtdCadernos = 3; }
        ...
        */
        bloco = (parseInt(Usuario.usu_login.replace(/\D/g, "")) % qtdCadernos) + 1;
...
function selecionarQuestionario(questionarioId) {
    try {
        ...
        */
        ...
        /**
        -----MSTECH-----
         *Questionários 1, 2, 3 e 12 são questionários padrão sobre dados pessoais e dia a dia das
         pessoas atuantes nas escolas. Veja detalhes:
         -Questionários 1: Questionário do Supervisor Escolar
         -Questionários 2: Questionário do Diretor de Escola
         -Questionários 3: Questionário do Coordenador Pedagógico
         -Questionários 12: Questionário do(a) Professor(a)
        ...
        */
        if ((questionarioId >= 1 && questionarioId <= 3) ||
            (questionarioId >= 12 && questionarioId <= 20)) {
            /**
            -----MSTECH-----
    ...
    carregarDataEscola(
        function () {
            try {
                ...
                /**
                -----MSTECH-----
                 *Lembrando que array de questionário é [1,2,3,8,9,10,11,12].
...
function validarInputs() {
    try {
        if (questionarioId_atual == 2) { //DIRETOR
            ...
            var qt15A = parseInt($("#Questionario_2_Questao_15_A").val());
            var qt15B = parseInt($("#Questionario_2_Questao_15_B").val());

            //Validação simples de todos os itens do questionário de diretor
            for (var i = 0; i < inputsDiretor.length; i++) {
                var itemDiretorAtual = $("#Questionario_2_Questao_" + inputsDiretor[i].qtID).val();
        ...
        else if (questionarioId_atual == 3) { //COORDENADOR
            //Items fixados no aplicativo para acesso também Offline
...
function definirEventHandlers() {
    ...
    */
    $("#btnQuestionario1_Iniciar,#btnQuestionario2_Iniciar,#btnQuestionario3_Iniciar," +
        "#btnQuestionario12_Iniciar,#btnQuestionario13_Iniciar,#btnQuestionario14_Iniciar," +
        "#btnQuestionario15_Iniciar,#btnQuestionario16_Iniciar,#btnQuestionario17_Iniciar," +
        "#btnQuestionario18_Iniciar,#btnQuestionario19_Iniciar,#btnQuestionario20_Iniciar").unbind("click").click(function () {
            try {
    ...
    */
    $("#Questionario_2_Questao_5_A,#Questionario_2_Questao_5_B,#Questionario_2_Questao_5_C,#Questionario_2_Questao_5_D,#Questionario_2_Questao_5_E").unbind("click").click(function () {
        try {
            $("#Questionario_2_Questao_5_F").prop('checked', false).checkboxradio('refresh');
        }
        catch (error) {
            console.log(error);
        }
    ...
    $("#Questionario_2_Questao_5_F").unbind("click").click(function () {
        try {
            $("#Questionario_2_Questao_5_A,#Questionario_2_Questao_5_B,#Questionario_2_Questao_5_C,#Questionario_2_Questao_5_D,#Questionario_2_Questao_5_E").prop('checked', false).checkboxradio('refresh');
        }
        catch (error) {
            console.log(error);
        }
    ...
    $("#Questionario_12_Questao_8_A,#Questionario_12_Questao_8_B,#Questionario_12_Questao_8_C,#Questionario_12_Questao_8_D,#Questionario_12_Questao_8_E").unbind("click").click(function () {
        try {
            $("#Questionario_12_Questao_8_F").prop('checked', false).checkboxradio('refresh');
        }
        catch (error) {
            console.log(error);
        }
    ...
    $("#Questionario_12_Questao_8_F").unbind("click").click(function () {
        try {
            $("#Questionario_12_Questao_8_A,#Questionario_12_Questao_8_B,#Questionario_12_Questao_8_C,#Questionario_12_Questao_8_D,#Questionario_12_Questao_8_E").prop('checked', false).checkboxradio('refresh');
        }
        catch (error) {
            console.log(error);
        }
    ...
    $("#Questionario_13_Questao_6_A,#Questionario_13_Questao_6_B,#Questionario_13_Questao_6_C,#Questionario_13_Questao_6_D,#Questionario_13_Questao_6_E").unbind("click").click(function () {
        try {
            $("#Questionario_13_Questao_6_F").prop('checked', false).checkboxradio('refresh');
        }
        catch (error) {
            console.log(error);
        }
    ...
    $("#Questionario_13_Questao_6_F").unbind("click").click(function () {
        try {
            $("#Questionario_13_Questao_6_A,#Questionario_13_Questao_6_B,#Questionario_13_Questao_6_C,#Questionario_13_Questao_6_D,#Questionario_13_Questao_6_E").prop('checked', false).checkboxradio('refresh');
        }
        catch (error) {
            console.log(error);
        }
...
function divResultadoProva (opcaoSelecionada) {
    try {
        for (var i = 0; i < 4; i++) {
            document.getElementById("resultados_div" + i).style.display = "none";
