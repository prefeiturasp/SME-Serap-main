...
namespace GestaoEscolar.Repository
{
    public class SYS_UnidadeAdministrativaRepository : ConnectionReadOnly, ISYS_UnidadeAdministrativaRepository
    {
        public IEnumerable<SYS_UnidadeAdministrativa> LoadSimple(Guid ent_id, IEnumerable<string> uad_id = null)
        {
            ...
            sql.Append("FROM SYS_UnidadeAdministrativa uad WITH (NOLOCK) ");
        ...
        public IEnumerable<SYS_UnidadeAdministrativa> LoadSimpleCoordinator(Guid ent_id, IEnumerable<string> uad_id)
		{
			...
			sql.AppendLine("FROM ESC_Escola WITH (NOLOCK) e ");
			sql.AppendLine("INNER JOIN SYS_UnidadeAdministrativa WITH (NOLOCK) uad ON e.uad_idSuperiorGestao = uad.uad_id ");
        ...
        public IEnumerable<SYS_UnidadeAdministrativa> LoadSimpleTeacher(Guid ent_id, Guid pes_id)
        {
            ...
            sql.Append("FROM ACA_Docente a WITH (NOLOCK) ");
            sql.Append("INNER JOIN TUR_TurmaDocente td WITH (NOLOCK) ON a.doc_id = td.doc_id ");
            sql.Append("INNER JOIN TUR_TurmaDisciplina tdis WITH (NOLOCK) ON tdis.tud_id = td.tud_id ");
            sql.Append("INNER JOIN TUR_Turma t WITH (NOLOCK) ON t.tur_id = tdis.tur_id ");
            sql.Append("INNER JOIN ESC_Escola e WITH (NOLOCK) ON e.esc_id = t.esc_id ");
            sql.Append("INNER JOIN SYS_UnidadeAdministrativa uad WITH (NOLOCK) ON uad.uad_id = e.uad_idSuperiorGestao ");
