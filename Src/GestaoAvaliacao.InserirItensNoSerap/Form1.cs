...
namespace InserirItensNoSerap
{
    public partial class Form1 : Form
    {
        ...
        {
            var itemBusiness = container.Resolve<IItemBusiness>();
            var directoryRoot = $@"C:\Users\jdulley.freitas\Downloads\SME_Videos_Libras\SME_Videos_Libras";
            var folders = Directory.GetDirectories(directoryRoot);
            foreach (var folder in folders)
            {
                var files = Directory.GetFiles(folder);
                var sequenciasImportadas = GetSequenciasImportadasLibras();

                foreach (var file in files)
                {
                    var fileNameWithoutExtension = Path.GetFileNameWithoutExtension(file);
                    //if (sequenciasImportadas.Any(s => s == int.Parse(fileNameWithoutExtension)))
                    //    continue;
                    SaveVideoLibras(itemBusiness, file, fileNameWithoutExtension);
                }

            }
        }

        private void SaveVideoLibras(IItemBusiness itemBusiness, string file, string fileNameWithoutExtension)
        {
            try
            {
                var item = itemBusiness.GetItemByItemCode(fileNameWithoutExtension);
                var video = UploadVideo(file, fileNameWithoutExtension);
                var thumbnail = UploadThumbnail(fileNameWithoutExtension);
                var itemFile = new ItemFile();
                if (video.Id > 0)
                    itemFile.File = new EntityFile { Id = video.Id, Path = video.Path };
                if (thumbnail.Id > 0)
                    itemFile.Thumbnail = new EntityFile { Id = thumbnail.Id, Path = thumbnail.Path };

                item.ItemFiles.Add(itemFile);
                item = itemBusiness.Update(item.Id, item);

                if (!item.Validate.IsValid)
                    SalvarLog(item.Validate.Message, int.Parse(fileNameWithoutExtension), Path.GetFileName(file), "Validação upload do video", null);
            }
            catch (Exception ex)
            {
                SalvarLog(ex.Message, int.Parse(fileNameWithoutExtension), Path.GetFileName(file), "Upload Video e Thumbnail", null, null);
            }
        }
        ...
        private EntityFile UploadThumbnail(string fileNameWithoutExtension)
        {
            var fileBusiness = container.Resolve<IFileBusiness>();
            var entity = new EntityFile();
            var file = @"C:\Users\jdulley.freitas\Downloads\SME_Videos_Libras\SME_Videos_Libras\serap.png";
            ...
            try
            {
                ...
                if (!File.Exists(file))
                {
                    SalvarLog($"Imagem não localizada. Nome do arquivo: {fileName}", int.Parse(fileNameWithoutExtension), fileName, "Upload Thumbnail", null);
                    return new EntityFile();
                }
                using (FileStream itemFile = System.IO.File.Open(file, FileMode.Open))
                {
            ...
            {
                SalvarLog($"Erro ao realizar upload da imagem:{ex.Message}", int.Parse(fileNameWithoutExtension), fileName, $"Upload Thumbnail", null);
                return new EntityFile();
            }
        }

        private EntityFile UploadVideo(string file, string fileNameWithoutExtension)
        {
            var fileBusiness = container.Resolve<IFileBusiness>();
            var entity = new EntityFile();
            var fileName = Path.GetFileName(file);
            ...
            try
            {
                ...
                if (!File.Exists(file))
                {
                    SalvarLog($"Video não localizado. Nome do arquivo: {fileName}", int.Parse(fileNameWithoutExtension), fileName, "Upload Video", null);
                    return new EntityFile();
                }
                using (FileStream itemFile = System.IO.File.Open(file, FileMode.Open))
                {
            ...
            {
                SalvarLog($"Erro ao realizar upload do video:{ex.Message}", int.Parse(fileNameWithoutExtension), fileName, $"Upload Video", null);
                return new EntityFile();
            }
        }
        ...
        private List<int> GetSequenciasImportadasLibras()
        {
            return new List<int>
            {
               18341,
