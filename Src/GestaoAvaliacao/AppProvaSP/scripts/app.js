...
$('.link-exportar-pdf-detalhes').click(function () {
    ...
    montarPaginaDoGraficoDeBarrasDoPdf(pdf);
});

function consultaDREParaGerarOPdf() {
    var Edicao = $("#ddlResultadoAlunoEdicao").val();
    var AreaConhecimentoID = $("#ddlResultadoAlunoAreaConhecimento").val();
    var alu_matricula = Usuario.usu_login.replace("RA", "");
...
function gerarRelatorio(ciclo, edicao, areaConhecimentoId, ano, divResultadoContainer, dataResultado, objetoEnviado) {
         $("#" + divResultadoContainer + " .divChartResultadoDetalhe").empty().append("<canvas id='chartResultadoDetalhe'></canvas>");
...
function montarRadarNoPdf(pdf) {
    ...
    listaDeAgregacao.forEach(agregacaoId => {
        ...
        pdf.addImage($("#" + agregacaoId)[0].toDataURL("image/png", 1.0), 'PNG', 60, canvasPosition, 450, 225);

        quantidade++;
    });
}

function montarPaginaDoGraficoDeBarrasDoPdf(pdf) {
    var srcImg = document.getElementById('chartResultadoDetalhe');

    var heightOfPage = 865;
    var widthOfPage = 560;

    for (var i = 0; i <= srcImg.height / heightOfPage; i++) {
        var sX = 0;
        var sY = heightOfPage * i;
        var sWidth = widthOfPage;
        var sHeight = heightOfPage;
        var dX = 0;
        var dY = 0;
        var dWidth = widthOfPage;
        var dHeight = heightOfPage;

        pdf.addPage();
        var canvasPosition = 15;
        pdf.fromHTML($('#divResultadoTituloDetalhe')[0], 125, canvasPosition);

        var canvas = document.getElementById('chartResultadoEscalaSaeb_1');
        var imgData = canvas.toDataURL("image/png", 1.0);
        canvasPosition = 60;
        pdf.addImage(imgData, 'PNG', 92, canvasPosition, 415, 75);

        window.onePageCanvas = document.createElement("canvas");
        onePageCanvas.setAttribute('width', widthOfPage);
        onePageCanvas.setAttribute('height', heightOfPage);
        var ctx = onePageCanvas.getContext('2d');
        ctx.drawImage(srcImg, sX, sY, sWidth, sHeight, dX, dY, dWidth, dHeight);

        canvasPosition = 140;
        pdf.addImage(onePageCanvas.toDataURL("image/png", 1.0), 'PNG', 92, canvasPosition, 421, 648.75);
    }
