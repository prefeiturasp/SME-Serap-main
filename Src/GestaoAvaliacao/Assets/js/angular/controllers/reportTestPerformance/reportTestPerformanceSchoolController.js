(function (angular, $) {
    ...
    angular
		.module('appMain')
		.controller("ReportTestPerformanceSchoolController", ReportTestPerformanceSchoolController);

    ReportTestPerformanceSchoolController.$inject = ['$scope', 'ReportTestPerformanceModel', 'TestModel', 'DisciplineModel', 'AdherenceModel', '$notification', '$timeout', '$pager', '$util'];

    function ReportTestPerformanceSchoolController($scope, ReportTestPerformanceModel, TestModel, DisciplineModel, AdherenceModel, $notification, $timeout, $pager, $util) {
        ...
        function configVariables() {
            // :paginação
            $scope.paginate = $pager(ReportTestPerformanceModel.getSchools);
            $scope.totalItens = 0;
            $scope.pages = 0;
            $scope.pageSize = 10;
            $scope.message = false;
        ...
        function getHeaderDetails(_callback) {
            TestModel.getInfoUadReport({
                ...
                if (result.success) {
                    ...
                    $notification[result.type ? result.type : 'error'](result.message);
                    $timeout(function () {
                        window.location.href = "/ReportTestPerformance/"
                    }, 3000);
                }
                if (_callback) _callback();
            });
            ...
            TestModel.getInfoTestCurriculumGrade({ Test_id: $scope.params.test_id }, function (result) {
                if (result.success) {
        ...
        */
        ...
        $scope.generateReport = function __generateReport() {
            ReportTestPerformanceModel.exportSchool($scope.searcheableFilter, function (result) {
                if (result.success) {
