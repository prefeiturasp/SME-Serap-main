...
(function (angular, $) {
    ...
    angular
		.module('appMain')
		.controller("BatchDetailsController", BatchDetailsController);


    BatchDetailsController.$inject = ['$rootScope', '$scope', '$notification', '$timeout', '$sce', '$pager', '$compile', '$util', '$window', 'AnswerSheetModel', 'AdherenceModel', 'FileModel', 'TestListModel', 'ParameterModel', '$interval'];
     ...
     * @return
     */
    function BatchDetailsController($rootScope, $scope, $notification, $timeout, $sce, $pager, $compile, $util, $window, AnswerSheetModel, AdherenceModel, FileModel, TestListModel, ParameterModel, $interval) {
        ...
        (function configUploadQueueStatus() {
            ...
            $scope.uploadQueueStatus = 1;
            $scope.pagerUploadQueue = {
                paginate: $pager(AnswerSheetModel.getUploadQueueStatus),
                pages: 0,
                totalItens: 0,
                message: false,
                pageSize: 10,
        ...
        $scope.showFiles = function showFiles(_batchQueueFileId, _fileName) {
        ...
        $scope.printFilter = function printFilter(label) {
            ...
            if (label == $scope.filters.StartDate.label
                || label == $scope.filters.EndDate.label) {
            ...
            if (label == $scope.filters.BatchQueueFile.label)
                ...
                activeFilterString += " " + $scope.filters.DRE.value.Description;
            else if (label == $scope.filters.School.label)
                activeFilterString += " " + $scope.filters.School.value.Description;
        ...
        $scope.removeFilter = function removeFilter(label) {
            if (label == $scope.filters.BatchQueueFile.label) {
                ...
                $scope.filters.BatchQueueFile.state = false;
            }
            else if (label == $scope.filters.DRE.label) {
        ...
        function getUploadQueueStatus() {
            ...
            $scope.pagerUploadQueue.paginate.paginate({ Situation: $scope.uploadQueueStatus }).then(function (result) {
                if (result.success) {
        ...
        function getFilter() {
            if ($scope.testInformation) {
                ...
                $scope.currentFilter = {
                    ...
                    BatchQueueId: ($scope.filters.BatchQueueFile && $scope.filters.BatchQueueFile.value) ? $scope.filters.BatchQueueFile.value.Id : 0,
                    SupAdmUnitId: ($scope.filters.DRE !== undefined && $scope.filters.DRE.value !== null) ? $scope.filters.DRE.value.Id : null,
                    SchoolId: ($scope.filters.School ? $scope.filters.School.value.Id : null),
                    CurriculumTypeId: $scope.filters.Year ? $scope.filters.Year.CurriculumTypeId : null,
                    ShiftTypeId: $scope.filters.Turn ? $scope.filters.Turn.Id : null,
                    StartDate: $scope.filters.StartDate.value,
                    EndDate: $scope.filters.EndDate.value,
        ...
        $scope.countFiltersSelected = function __countFiltersSelected() {
            ...
            for (i = 0; i < max; i++) {
                if ($scope.filters[list[i]].value && $scope.filters[list[i]].state) {
                    count++;
                }
            }
