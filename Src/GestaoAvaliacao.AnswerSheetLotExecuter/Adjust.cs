...
namespace GestaoAvaliacao.AnswerSheetLotExecuter
{
    public partial class Adjust : Form
    {
                                ...
                                foreach (var turma in turmas)
                                {
                                    ...
                                    try
                                    {
                                        if (dadosCompletosTurma.CodigoTurma > 0)
                                        {
                                            ...
                                            sqlQuery.AppendLine("					tme.tme_id");
                                            sqlQuery.AppendLine("	FROM GestaoAvaliacao_SGP.dbo.ACA_TipoModalidadeEnsino tme WITH (NOLOCK)");
                                            sqlQuery.AppendLine("	WHERE tme.tme_nome = @tme_nome AND");
        ...
        private async void ButtonGerarPercentualRespostas_Click(object sender, EventArgs e)
        {
            ...
            sqlQuery.AppendLine("	FROM [Test] t WITH (NOLOCK)");
            sqlQuery.AppendLine("		INNER JOIN [Block] b WITH (NOLOCK)");
            sqlQuery.AppendLine("			ON t.Id = b.Test_Id");
            sqlQuery.AppendLine("		INNER JOIN [BlockItem] bi WITH (NOLOCK)");
            sqlQuery.AppendLine("			ON b.Id = bi.Block_Id");
            sqlQuery.AppendLine("		INNER JOIN [Item] i WITH (NOLOCK)");
            sqlQuery.AppendLine("			ON bi.Item_Id = i.Id");
            sqlQuery.AppendLine("		inner join [Alternative] a WITH (NOLOCK)");
            sqlQuery.AppendLine("			on i.Id = a.Item_Id");
            sqlQuery.AppendLine("		INNER JOIN TestCurriculumGrade tcg WITH (NOLOCK)");
            sqlQuery.AppendLine("			ON t.Id = tcg.Test_Id");
            sqlQuery.AppendLine("		INNER JOIN SGP_TUR_TurmaCurriculo tc WITH (NOLOCK)");
            sqlQuery.AppendLine("			ON tcg.TypeCurriculumGradeId = tc.tcp_id");
            sqlQuery.AppendLine("		INNER JOIN SGP_TUR_Turma tur WITH (NOLOCK)");
            sqlQuery.AppendLine("			ON tc.tur_id = tur.tur_id AND");
            sqlQuery.AppendLine("			   tur.tur_situacao <> @stateDelete");
            sqlQuery.AppendLine("		INNER JOIN SGP_ESC_Escola e WITH (NOLOCK)");
